...
class ProbeView(WidgetBase):
    ...
    def initialize_plot(self):
        ...
        for i, channel_id in enumerate(self.controller.channel_ids):
            ...
            label.setPos(contact_positions[i, 0], contact_positions[i, 1])
            self.plot.addItem(label)
            self.channel_labels.append(label)

        
        x, y = contact_positions.mean(axis=0)
        self.roi = pg.CircleROI([x-10, y-10], [20, 20],  pen='#7F7F0C') #pen=(4,9),
        self.plot.addItem(self.roi)
        
        self.roi.sigRegionChanged.connect(self.on_roi_change)
        
        xlim0 = np.min(contact_positions[:, 0]) - 20
        xlim1 = np.max(contact_positions[:, 0]) + 20
        ylim0 = np.min(contact_positions[:, 1]) - 20
        ylim1 = np.max(contact_positions[:, 1]) + 20
